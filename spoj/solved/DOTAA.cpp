...
int main()
{
	long long int towerCount,heroesCount;
	long long int towerDamagePower;
	//long long int heroPower[20001];
	long long int testCases,i;
	long long currentPower;
	//long long int lessPowerHeroes;
	//long long int greatPowerHeroes[20001];
	//long long int greatPowerHeroesCount;
	long long int towerPassed;

	scanf("%lld",&testCases);
	
	while( testCases-- )
	{
		scanf("%lld %lld %lld",&heroesCount,&towerCount,&towerDamagePower);

		//greatPowerHeroesCount = 0;
		//lessPowerHeroes = 0;
		towerPassed = 0;

		for( i=0; i<heroesCount; i++ )
		{
			scanf("%lld",&currentPower);
			if( currentPower % towerDamagePower != 0 )
				towerPassed = towerPassed + (long long int )(floorl(currentPower/towerDamagePower));
			else
			{
				towerPassed = towerPassed + currentPower/towerDamagePower  -1 ;
			}
			//printf("towerPassed:%lld,currentPower:%lld,towerDamagePower:%lld\n",towerPassed,currentPower,towerDamagePower);
			/*if( currentPower < towerDamagePower )
				lessPowerHeroes++;
			else
				greatPowerHeroes[greatPowerHeroesCount++] = currentPower;*/
		}

		/*for( i=0; i<greatPowerHeroes; i++)
		{
			towerPassed += towerDamagePower/greatPowerHeroes[i];
		}*/
		if( towerPassed >= towerCount )
			printf("YES\n");
		else
			printf("NO\n");
	}

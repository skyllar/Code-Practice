...
void recur(int *a, int sumTillHere, int left, int index, int size) {

	//printf("\n$$$$\n");
	if (index > size)
		return;

	if (index == size && left > 0)
		return;

	if (left == 0) {

		int temp = totalSum - sumTillHere;
		if (minDiffrence > temp) {
			minDiffrence = temp;
		}
		return;
	}

	recur(a, sumTillHere + a[index], left - 1, index + 1, size);
	recur(a, sumTillHere, left, index + 1, size);
}
